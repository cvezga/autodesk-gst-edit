...
public abstract class AbstractDst2Xml
{

    static final char[] encode = new char[] {
    ...
    static final char[] decode = new char[] {
    ...
    static String decode( byte[] bytes, int len )
    {
        ...
        Arrays.stream( arraytoObject( bytes ), 0, len )
                .forEach( b -> sb.append( getChar( b, decode ) ) );

        return sb.toString();
    }

    private static Byte[] arraytoObject( byte[] bytes )
    {
    ...
    {
        return getChar( b, encode );
    }

    private static char getChar( byte b, char[] charMap )
    {
